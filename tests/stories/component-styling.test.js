...
describe('Story 3.5: Component Styling Consistency', () => {
  ...
  describe('Design System Documentation - CSS Structure', () => {
    ...
    it('should define the lean bottom live strip without fake telemetry controls', () => {
      ...
      expect(cssContent).toContain('.layout-grid-control')
      expect(cssContent).toContain('.layout-axis-control')
      expect(cssContent).toContain('.layout-step-btn')
    ...
    it('should define stage layout surfaces for multi-slot playback', () => {
      ...
      expect(cssContent).toContain('--stage-layout-columns')
      expect(cssContent).toContain('--stage-layout-rows')
      expect(cssContent).toContain(
        'grid-template-columns: repeat(var(--stage-layout-columns, 1), minmax(0, 1fr))'
      )
    ...
    it('should keep active toasts above the bottom live strip', () => {
      ...
      expect(cssContent).toContain('.ui-idle .toast-container')
      expect(cssContent).toContain('.ui-active .toast-container')
      expect(cssContent).toContain('bottom: 20px')
    })
