...
- **Primary Goals (MVP):**
    1.  Enable users to instantly initiate an endless, randomized playback of their local photos (JPG/PNG/GIF) and videos (MP4/MOV/WebM) by simply dropping files or selecting via a file dialog.
    2.  Implement a core playback engine that seamlessly transitions between media segments (2-6 seconds each) using hard cuts, ensuring a continuous and unpredictable visual flow.
    ...
    5.  Ensure all user configurations (file selections - if possible via API, text entries, timing settings, advanced display settings, video segment controls) are automatically saved and loaded from `localStorage` for persistence across sessions.
...
*   **Media Input:**
    *   Ability to add media (photos: JPG, PNG, GIF; videos: MP4, MOV, WebM) via drag-and-drop of files/folders or a native file picker dialog.
...
*   **Playback Engine:**
    ...
    *   Fixed segment duration (configurable slider, defaulting to 2-6 seconds range).
...
*   **Advanced Display & Projection Tools (UI Toggleable Panel):**
    ...
    *   Define Projection Area Size/Aspect Ratio & Fixed Stage (Story E5.S4 complexity). Controls for presets (16:9, 4:3, 1:1) and custom input.
...
*   **Persistence:**
    *   All user settings (timing, text entries, advanced display settings, video segment controls) AND advanced UI state (panel visible/hidden, projection mode active/inactive) automatically saved to and loaded from `localStorage`.
    *   Attempt to persist file references using `FileSystemAccessAPI` if browser supports, otherwise just settings/UI state persist.
*   **UI/UX:**
...
1.  **Story:** As a User, I want to be able to drop image and video files/folders onto the application window so that I can easily load my media.
    *   **AC 1.1:** Given I am on the VJ Tam Tam page, when I drag one or more image files (JPG, PNG, GIF) from my file system and drop them anywhere onto the application window, then the application processes these files for playback.
    ...
    *   **AC 1.4:** Given I am on the VJ Tam Tam page, when I drag a file type that is *not* supported (e.g., a `.txt` or `.pdf` file) and drop it, then the application ignores this file **AND a toast notification appears indicating that the file type is not supported and listing the supported types (JPG, PNG, GIF, MP4, MOV, WebM).**
...
2.  **Story:** As a User, I want to be able to select image and video files/folders using a standard file picker dialog so that I have an alternative way to load my media if drag-and-drop is inconvenient.
    ...
    *   **AC 2.2:** Given the file picker is open, when I select one or more supported image files (JPG, PNG, GIF) and confirm the selection, then the application processes these files for playback.
...
7.  **Story:** As a User, when the media pool is empty, I want to see an informative message guiding me on how to add media so that I know how to get started.
    *   **AC E1.S7.1:** Given the application loads and the media pool is empty (either initially or after clearing media), then a specific text message, like "Welcome to VJ Tam Tam, please drag and drop some photos and videos to get started with your performance", is displayed prominently on the stage area.
...
*Goal: Enhance the playback engine to handle random segments (2-6s) of media files, including skipping parts of videos, providing the "eternal slideshow" effect.*

1.  **Story:** As the Application, I want to select a random segment (2-6 seconds) from the currently chosen media item (image or video) for display so that the playback is constantly varied and unpredictable.
    ...
    *   **AC 1.2:** The random start point is calculated such that a segment of the configured duration (defaulting to 2-6 seconds initially) can be played without exceeding the media's total duration.
    *   **AC 1.3:** The calculated segment duration for display is a random value between the configured minimum and maximum duration settings (initially defaulting to a range like 2-6 seconds).
...
2.  **Story:** As the Application, when displaying a video segment, I want to ensure it plays correctly for its specified duration (2-6 seconds) starting from the randomly selected point so that video playback is consistent with images.
...
As discussed and agreed upon (and documented in `bmad-agent/data/project-memory/vj-tam-tam-tech-choices.md`), the core technical decisions for this project are:

-   **Frontend Stack:** Pure Vanilla JS + ES Modules (no frameworks).
-   **Rendering:** HTML/CSS for the stage (`<div>` based) rather than Canvas for core media display (chosen for better text rendering and simpler media handling, acknowledging CSS filter limitations).
-   **Key Libraries:** Maptastic (local copy in `lib/`) for projection warping; native `FileSystemAccessAPI` for file handling.
-   **Code Structure:** Modular (`core.js`, `mapping.js`, `ui.js` in `src/`), flat directory tree at repository root, `docs/` for specifications. Avoid over-engineering (e.g., minimal functional style).
-   **Deployment:** GitHub Pages.
-   **Analytics/Tracking:** Zero analytics or tracking.

**Detailed Application File Structure (relative to repository root):**
```plaintext
./
...
│   └── maptastic.js
├── src/
│   ├── core.js         # Media pool, randomization
│   ├── mapping.js      # Warping/filters
│   └── ui.js           # Idle UI, controls
└── assets/
    ├── media/
    └── test-card.png
```
