...
- **Primary Goals (MVP):**
  ...
  2.  Implement a core playback engine that seamlessly transitions between media segments (with an initial default duration of 5 seconds each, individually configurable from 1 to 300 seconds) using hard cuts, ensuring a continuous and unpredictable visual flow.
...
_Goal: Enhance the playback engine to handle random segments (configurable duration, e.g., default 5s, range 1-300s) of media files, including skipping parts of videos, providing the "eternal slideshow" effect._
...
3.  **Story:** As a User, I want to be able to configure the duration range (minimum and maximum seconds) for the random segments so that I can control the pace of the visual transitions.
    ...
    - **AC E2.S3.2a (Min Slider Range & Default):** The minimum segment duration control (slider/input) allows selection from 1 second to 300 seconds, and its initial default value upon first load (before any user configuration is saved) is 5 seconds.
    - **AC E2.S3.2b (Max Slider Range & Default):** The maximum segment duration control (slider/input) allows selection from 1 second to 300 seconds, and its initial default value upon first load (before any user configuration is saved) is 5 seconds.
