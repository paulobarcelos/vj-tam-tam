...
#### 15.11. Architectural Quality Patterns (Critical)

These patterns are **mandatory** and violations should be treated as blocking issues:

- **State Management Consistency:**
  - **Rule:** ALL application state MUST flow through the stateManager. 
  - **Anti-Pattern:** Direct localStorage access bypassing stateManager.
  - **Correct Pattern:** Use `stateManager.updateUISettings()` instead of `localStorage.setItem()`.
  - **Rationale:** Ensures consistent state management, event emission, and persistence patterns.

- **DRY Principle (Don't Repeat Yourself):**
  - **Rule:** Common operations must be abstracted into reusable functions.
  - **Anti-Pattern:** Copying DOM manipulation code across multiple methods.
  - **Correct Pattern:** Create centralized methods like `updateSegmentControlsDOM()` for repeated operations.
  - **Rationale:** Reduces bugs, improves maintainability, ensures consistent behavior.

- **Event-Based Architecture:**
  - **Rule:** Use eventBus for inter-module communication, not direct method calls.
  - **Anti-Pattern:** Calling `uiManager.updateSomething()` directly from stateManager.
  - **Correct Pattern:** Emit events like `state.uiSettingsUpdated` and let modules listen.
  - **Rationale:** Maintains loose coupling and allows multiple modules to react to state changes.

- **Separation of Concerns:**
  - **Rule:** Each module has a single, well-defined responsibility.
  - **Anti-Pattern:** UI logic mixed with business logic in the same function.
  - **Correct Pattern:** stateManager handles data, uiManager handles DOM, separate validation logic.
  - **Rationale:** Improves testability and reduces complexity.

- **Initialization Order Dependencies:**
  - **Rule:** Modules must not assume other modules are fully initialized during their own init().
  - **Anti-Pattern:** Calling `stateManager.getSettings()` in `uiManager.init()` before stateManager restoration.
  - **Correct Pattern:** Use event-driven initialization with fallback patterns.
  - **Rationale:** Prevents timing bugs and race conditions.

#### 15.12. Code Quality Checklist

Before marking any story or task complete, verify:

- [ ] **No duplicate code** - Common operations are abstracted
- [ ] **State flows through stateManager** - No direct localStorage usage
- [ ] **Events used for communication** - No tight coupling between modules  
- [ ] **Single responsibility** - Each function has one clear purpose
- [ ] **Proper error handling** - All operations wrapped in try/catch where needed
- [ ] **Consistent patterns** - New code follows existing architectural patterns
- [ ] **Tests pass** - All existing tests continue to work
- [ ] **Linter clean** - No ESLint errors or warnings

#### 15.13. Refactoring Guidelines

When refactoring existing code:

- **Identify patterns first** - Look for repeated code blocks
- **Abstract incrementally** - Don't over-engineer, focus on actual duplication
- **Maintain backward compatibility** - Don't break existing event contracts
- **Test each step** - Run tests after each refactoring step
- **Document changes** - Update JSDoc for modified public APIs
