...
- [ ] Task 1: Implement Idle/Active State Management System (AC: 2.1, 2.2, 2.3)
  - [ ] Subtask 1.1: Add idle/active state properties to UIManager class (isUIIdle, idleTimer, IDLE_TIMEOUT_MS)
  - [ ] Subtask 1.2: Create methods to handle idle state transitions (enterIdleState, exitIdleState, resetIdleTimer)
  - [ ] Subtask 1.3: Implement activity detection for mouse movement and keyboard input events
  - [ ] Subtask 1.4: Add special handling to exclude ESC key in fullscreen mode from activity detection
  - [ ] Subtask 1.5: Set up automatic idle timer (default 3-5 seconds of inactivity) with configurable timeout

- [ ] Task 2: Implement UI Element Visibility Control (AC: 2.1, 2.2)
  - [ ] Subtask 2.1: Add CSS classes for idle state (.ui-idle) and element visibility (.hidden-on-idle)
  - [ ] Subtask 2.2: Apply visibility control to left drawer with smooth transitions
  - [ ] Subtask 2.3: Prepare for future Maptastic handles and test card visibility control (placeholder implementation)
  - [ ] Subtask 2.4: Ensure stage content remains fully visible and interactive during idle state

- [ ] Task 3: Integrate Activity Detection with Existing UI Systems (AC: 2.2, 2.4)
  - [ ] Subtask 3.1: Integrate activity detection with existing drag-and-drop system
  - [ ] Subtask 3.2: Ensure file picker interactions properly trigger active state
  - [ ] Subtask 3.3: Coordinate with existing advanced controls and drawer state management
  - [ ] Subtask 3.4: Maintain compatibility with existing event bus communications

- [ ] Task 4: Implement Session Behavior and State Persistence (AC: 2.4)
  - [ ] Subtask 4.1: Ensure idle state is not persisted to localStorage (transient state only)
  - [ ] Subtask 4.2: Implement proper initialization behavior on page reload (start in active state)
  - [ ] Subtask 4.3: Apply first-interaction logic to transition from initial active to idle management
  - [ ] Subtask 4.4: Verify that persisted UI settings (advanced controls visibility) are restored correctly

- [ ] Task 5: Testing and Edge Case Handling (All ACs)  
  - [ ] Subtask 5.1: Test idle/active transitions with different types of user interactions
  - [ ] Subtask 5.2: Verify ESC key exclusion in fullscreen mode works correctly
  - [ ] Subtask 5.3: Test interaction with existing features (media playback, drag-drop, file picker)
  - [ ] Subtask 5.4: Validate smooth transitions and no flickering or performance issues
  - [ ] Subtask 5.5: Test cross-browser compatibility for activity detection and timers

- [ ] Task 6: Performance and Accessibility Validation (AC: 2.2, 2.3)
  - [ ] Subtask 6.1: Ensure activity detection doesn't impact performance during media playback
  - [ ] Subtask 6.2: Verify that idle state transitions don't interfere with keyboard navigation
  - [ ] Subtask 6.3: Test that UI remains accessible when transitioning between states
  - [ ] Subtask 6.4: Validate that the idle timeout provides adequate time for user interaction
