...
- **Deployment Strategy:** Manual deployment using **git subtree** to push the `/app` folder contents to the `gh-pages` branch. [cite: 92] The `npm run publish` script regenerates the content-versioned service worker and then executes `git subtree push --prefix app origin gh-pages`. [cite: 93]
- **Deployment Process:**
  ...
  5. Browsers that have already visited the app install the new service worker, precache the new app shell, and delete older VJ Tam Tam service worker caches on activation.
- **Environments:**
