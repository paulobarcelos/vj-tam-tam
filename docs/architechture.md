...
* **Code Comments:** Explain *why* complex or non-obvious code exists, not *what* it does.
...
* **Minimize Application Dependencies:** The core application deployed to GitHub Pages must remain Vanilla JS with minimal, locally-hosted libraries (like Maptastic.js in `/lib/`) that do not require a build step for the *application itself*.
...
* **Adding New Application Dependencies:** Introducing new external libraries for the *application itself* must be explicitly discussed, justified, and approved, adhering to the "minimal, locally-hosted, no-build-step for application" criteria.
